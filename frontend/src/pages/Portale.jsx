  ...
  const invia = async () => {
    ...
    try {
      const res = await api.post(
        `/api/portal/portale/firma/${contratto.id}`,
        {
      ...
      setErrore(
        e.response?.data?.message || e.response?.data?.detail || 'Errore durante la firma. Riprova.'
      );
    } finally {
      setLoading(false);
    }
    ...
    return (
      ...
      >
        <div style={{ textAlign: 'center', padding: '16px 0' }}>
          ...
          >
            <div>
              <strong>Hash firma:</strong> {firmataOk.hash_firma?.substring(0, 24)}...
            </div>
            <div>
              <strong>Data:</strong> {firmataOk.certificato?.data_firma} alle{' '}
              {firmataOk.certificato?.ora_firma}
            </div>
          </div>
          <button
      ...
      >
        <div>
          ...
          <p style={{ margin: '4px 0 0', fontSize: 13, color: '#64748b' }}>
            {contratto.tipo || contratto.nome || 'Contratto'} —{' '}
            {contratto.data_documento ? formatDateIT(contratto.data_documento) : ''}
          </p>
        </div>
        <div
          style={{
        ...
        {['lettura', 'firma'].map(s => (
          <button
            ...
            style={{
              ...
              borderRadius: 8,
              border: 'none',
              cursor: s === 'firma' && !scrollOk ? 'not-allowed' : 'pointer',
              fontWeight: 700,
              fontSize: 13,
        ...
        <>
          ...
          >
            ...
            <p style={{ margin: '0 0 10px' }}>
              Tra <strong>Ceraldi Group S.r.l.</strong> (di seguito "Datore di lavoro") e il/la
              dipendente identificato/a tramite account Google associato a questo portale (di
              seguito "Dipendente").
            </p>
            {contratto.contenuto ? (
              ...
              <>
                <p style={{ margin: '0 0 10px' }}>
                  Il presente documento disciplina il rapporto di lavoro subordinato tra le parti,
                  in conformità alla normativa vigente (D.Lgs. 81/2008, CCNL di categoria
                  applicabile).
                </p>
                <p style={{ margin: '0 0 10px' }}>
                  <strong>Art. 1 – Decorrenza e durata.</strong> Il rapporto di lavoro decorre dalla
                  data indicata nel documento e si intende, salvo diversa indicazione, a tempo
                  indeterminato.
                </p>
                <p style={{ margin: '0 0 10px' }}>
                  <strong>Art. 2 – Mansioni.</strong> Il dipendente è assunto con le mansioni
                  indicate nell'allegato, con obbligo di osservare le disposizioni impartite dal
                  datore di lavoro.
                </p>
                <p style={{ margin: '0 0 10px' }}>
                  <strong>Art. 3 – Retribuzione.</strong> La retribuzione mensile lorda è quella
                  indicata nel sistema gestionale e nei cedolini paga allegati.
                </p>
                <p style={{ margin: '0 0 10px' }}>
                  <strong>Art. 4 – Orario di lavoro.</strong> L'orario di lavoro è quello previsto
                  dal CCNL applicato, salvo diverse disposizioni concordate per iscritto.
                </p>
                <p style={{ margin: '0 0 10px' }}>
                  <strong>Art. 5 – Obblighi del dipendente.</strong> Il dipendente si impegna a
                  svolgere le proprie mansioni con diligenza, a rispettare il segreto professionale
                  e a non divulgare informazioni riservate dell'azienda a terzi.
                </p>
                <p style={{ margin: '0 0 10px' }}>
                  <strong>Art. 6 – Privacy.</strong> Il trattamento dei dati personali avviene in
                  conformità al Regolamento EU 2016/679 (GDPR). Il dipendente ha il diritto di
                  accedere, rettificare e cancellare i propri dati nei termini di legge.
                </p>
                <p style={{ margin: 0 }}>
                  <strong>Art. 7 – Firma Elettronica.</strong> La firma digitale apposta in questa
                  sede costituisce firma elettronica semplice (FES) ai sensi dell'art. 3 del
                  Regolamento eIDAS (UE 910/2014), con valore legale equivalente a firma autografa
                  per i documenti contemplati dall'art. 21 del D.Lgs. 82/2005 (CAD).
                </p>
              </>
            )}
          </div>
          ...
          <button
            ...
            style={{
              ...
              fontWeight: 700,
              fontSize: 15,
              cursor: scrollOk ? 'pointer' : 'not-allowed',
            }}
          >
            Procedi alla Firma
          </button>
      ...
      {step === 'firma' && (
        <>
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            <SignaturePad onReady={b64 => setFirmaB64(b64)} onClear={() => setFirmaB64('')} />
          </div>

          {/* Checkboxes */}
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 16 }}>
            <label
              data-testid="check-letto"
          ...
          >
            La firma elettronica semplice (FES) ha valore legale ai sensi dell'art. 3 eIDAS (UE
            910/2014). Vengono registrati: IP, timestamp, user-agent, tempo di lettura e hash del
            documento.
          </div>
          ...
          {!canFirmare && (
            <div style={{ fontSize: 12, color: '#94a3b8', marginBottom: 12 }}>
              Mancano: {!nomeDigitato.trim() || nomeDigitato.length <= 3 ? 'nome, ' : ''}
          ...
          <button
            ...
            style={{
              ...
              fontWeight: 700,
              fontSize: 15,
              cursor: canFirmare && !loading ? 'pointer' : 'not-allowed',
              display: 'flex',
              alignItems: 'center',
              justifyContent: 'center',
              gap: 8,
  ...
  const loadPortalData = useCallback(async email => {
    ...
    try {
      ...
      const dipRes = await api
        .get(`/api/dipendenti/by-google-email?email=${encodeURIComponent(email)}`)
        .catch(() => ({ data: null }));
      ...
      >
        ...
        >
          ...
          <p style={{ color: '#475569', fontSize: 14, margin: '28px 0', lineHeight: 1.6 }}>
            Accedi con il tuo account Google aziendale per consultare cedolini, contratti e firmare
            documenti.
          </p>
        ...
        {contrattiDaFirmare.length > 0 && (
          ...
          >
            ...
            <span style={{ fontSize: 14, fontWeight: 600, color: '#92400e' }}>
              Hai {contrattiDaFirmare.length} documento{contrattiDaFirmare.length > 1 ? 'i' : ''} da
              firmare
            </span>
          </div>
        )}
                  ...
                  >
                    <div>
                      ...
                      <div style={{ fontSize: 12, color: '#64748b' }}>
                        {c.netto_pagato
                          ? `Netto: €${Number(c.netto_pagato).toLocaleString('it-IT', { minimumFractionDigits: 2 })}`
