      ...
      actions={
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          <button
            onClick={() =>
              window.open(
                `${api.defaults.baseURL}/api/iva/export/pdf/trimestrale/${selectedYear}/${Math.ceil(selectedMonth / 3)}`,
                '_blank'
              )
            }
            style={{
          ...
          <button
            onClick={() =>
              window.open(
                `${api.defaults.baseURL}/api/iva/export/pdf/annuale/${selectedYear}`,
                '_blank'
              )
            }
            style={{
    ...
    >
      ...
      {loading ? (
        ...
        <>
          ...
          {annualData && (
            <PageGrid cols={3} gap={16} style={{ marginTop: 20 }}>
              ...
              <KPICard
                ...
                value={formatEuro(annualData.totali?.iva_debito)}
                subtext={`${annualData.totali?.corrispettivi_count || 0} corrispettivi`}
                color="#ea580c"
                bgColor="#fff7ed"
                icon={TrendingUp}
              />
              <KPICard
                ...
                value={formatEuro(annualData.totali?.iva_credito)}
                subtext={`${annualData.totali?.fatture_count || 0} fatture`}
                color="#16a34a"
                bgColor="#dcfce7"
                icon={TrendingDown}
              />
          ...
          {viewMode === 'annual' && annualData && (
            <PageSection
              title={`Riepilogo IVA Annuale ${selectedYear}`}
                  ...
                  <tbody>
                    {(annualData.mesi || annualData.monthly_data)?.map((m, idx) => (
                      ...
                      >
                        ...
                        <td style={{ padding: '12px 16px', textAlign: 'right', color: '#ea580c' }}>
                          {formatEuro(m.iva_debito)}
                        </td>
                        <td
                          style={{
                            padding: '12px 10px',
                        ...
                        >
                          {(m.fatture_count || 0) + (m.note_credito_count || 0)}
                        </td>
                        <td
                          style={{
                            padding: '12px 16px',
                        ...
                        <td
                          style={{
                            padding: '12px 16px',
                            textAlign: 'right',
                            color: getSaldoColor(m.saldo_precedente || 0),
                            background: '#f8faff',
                            fontSize: 13,
                          }}
                        >
                          {m.saldo_precedente
                            ? (m.saldo_precedente > 0 ? '+' : '') + formatEuro(m.saldo_precedente)
                            : '—'}
                        </td>
                        <td
                          style={{
                            ...
                            textAlign: 'right',
                            fontWeight: 700,
                            color: getSaldoColor(m.saldo_progressivo || 0),
                            background: '#f8faff',
                          }}
                        >
                        ...
                        <td style={{ padding: '12px 16px', textAlign: 'center' }}>
                          <span
                            style={{
                              ...getSaldoBadge(m.stato_progressivo || m.stato),
                              padding: '4px 10px',
                              borderRadius: 6,
                              fontSize: 12,
                              fontWeight: 600,
                    ...
                    >
                      ...
                      <td
                        style={{
                          ...
                          background: '#f8faff',
                          color: getSaldoColor(
                            annualData.totali?.saldo_progressivo || annualData.totali?.saldo
                          ),
                        }}
                      >
                        {formatEuro(
          ...
          {viewMode === 'quarterly' && annualData && (
            <PageSection
              title={`Riepilogo IVA Trimestrale ${selectedYear}`}
              style={{ marginTop: 20 }}
            >
                ...
                {[1, 2, 3, 4].map(q => {
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        <div>
                          <div style={{ fontSize: 11, color: '#6b7280' }}>Debito</div>
                          <div style={{ fontSize: 16, fontWeight: 700, color: '#ea580c' }}>
                            {formatEuro(totDebito)}
                          </div>
                        </div>
                        <div>
                          <div style={{ fontSize: 11, color: '#6b7280' }}>Credito</div>
                          <div style={{ fontSize: 16, fontWeight: 700, color: '#16a34a' }}>
                            {formatEuro(totCredito)}
                          </div>
                        </div>
                      </div>
                      <div
                      ...
                      >
                        ...
                        <span
                          style={{ fontSize: 18, fontWeight: 700, color: getSaldoColor(saldo) }}
                        >
                          {formatEuro(saldo)}
                        </span>
                      </div>
          ...
          {viewMode === 'monthly' && monthlyData && (
            <PageSection
              title={`Dettaglio IVA ${mesiItaliani[selectedMonth]} ${selectedYear}`}
              style={{ marginTop: 20 }}
            >
              <PageGrid cols={3} gap={16}>
                <KPICard
                  label="IVA Debito"
                  value={formatEuro(monthlyData.totali?.iva_debito)}
                  subtext={`${monthlyData.totali?.corrispettivi_count || 0} corr.`}
                  color="#ea580c"
                  bgColor="#fff7ed"
                />
                <KPICard
                  label="IVA Credito"
                  value={formatEuro(monthlyData.totali?.iva_credito)}
                  subtext={`${monthlyData.totali?.fatture_count || 0} fatt.`}
                  color="#16a34a"
                  bgColor="#dcfce7"
                />
                <div
                ...
                >
                  ...
                  <div
                    style={{
                      fontSize: 24,
                      fontWeight: 700,
                      color: getSaldoColor(monthlyData.totali?.saldo),
                    }}
                  >
              ...
              {monthlyData.giorni?.filter(g => g.iva_debito > 0 || g.iva_credito > 0).length >
                0 && (
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                    <thead>
                      <tr style={{ borderBottom: '2px solid #e5e7eb', background: '#f9fafb' }}>
                        ...
                        <th style={{ padding: 10, textAlign: 'right' }}>Debito</th>
                        <th style={{ padding: 10, textAlign: 'right' }}>Credito</th>
                        <th style={{ padding: 10, textAlign: 'right' }}>Saldo</th>
                      </tr>
                    </thead>
                    <tbody>
                      {(monthlyData.giorni || [])
                        .filter(g => g.iva_debito > 0 || g.iva_credito > 0)
                        .map((g, idx) => (
                          <tr key={idx} style={{ borderBottom: '1px solid #f3f4f6' }}>
                            <td style={{ padding: 10 }}>
                              {g.giorno}/{selectedMonth}/{selectedYear}
                            </td>
                            <td style={{ padding: 10, textAlign: 'right', color: '#ea580c' }}>
                              {formatEuro(g.iva_debito)}
                            </td>
                            <td style={{ padding: 10, textAlign: 'right', color: '#16a34a' }}>
                              {formatEuro(g.iva_credito)}
                            </td>
                            <td
                              style={{
                                padding: 10,
          ...
          {viewMode === 'today' && todayData && (
            <PageSection
              title={`IVA Oggi - ${todayData.data || formatDateIT(new Date())}`}
              style={{ marginTop: 20 }}
            >
              <PageGrid cols={3} gap={16}>
                <KPICard
                  label="IVA Debito Oggi"
                  value={formatEuro(todayData.iva_debito)}
                  color="#ea580c"
                  bgColor="#fff7ed"
                  icon={TrendingUp}
                />
                <KPICard
                  label="IVA Credito Oggi"
                  value={formatEuro(todayData.iva_credito)}
                  color="#16a34a"
                  bgColor="#dcfce7"
                  icon={TrendingDown}
                />
                <KPICard
                  label="Saldo Oggi"
                  value={formatEuro(todayData.saldo)}
      ...
      <PageSection title="Come funziona il calcolo IVA" icon="ℹ️" style={{ marginTop: 20 }}>
        <ul style={{ margin: 0, paddingLeft: 20, lineHeight: 2, color: '#475569', fontSize: 13 }}>
          <li>
            <strong>IVA Debito</strong>: calcolata dai corrispettivi giornalieri (vendite)
          </li>
          <li>
            <strong>IVA Credito</strong>: estratta dalle fatture passive (acquisti)
          </li>
          <li>
            <strong>Saldo positivo</strong> = IVA da versare all'erario
          </li>
          <li>
            <strong>Saldo negativo</strong> = IVA a credito (compensabile)
          </li>
        </ul>
      </PageSection>
    </PageLayout>
