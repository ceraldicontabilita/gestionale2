...
export default function DizionarioArticoli() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const res = await api.get(
        `/api/dizionario-articoli/cerca?q=${encodeURIComponent(search)}&limit=100`
      );
  ...
  const handleCategorizzaAI = async () => {
    if (
      !confirm(
        'Vuoi usare Claude AI per categorizzare gli articoli non classificati? (max 50 alla volta)'
      )
    )
      return;
    try {
    ...
    >
      <div>
        ...
        {message && (
          ...
          >
            {message.text}
            <button onClick={() => setMessage(null)} style={{ marginLeft: 16, cursor: 'pointer' }}>
              ✕
            </button>
          </div>
        )}
        ...
        {stats && (
          ...
          >
            ...
            <StatCard
              ...
              value={stats.confidenza?.media || 0}
              color="#f59e0b"
            />
            <StatCard
              label="Non Classificati"
              value={stats.confidenza?.non_classificati || 0}
              color="#ef4444"
            />
          </div>
        )}
        ...
        >
          ...
          <label style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <input
              ...
              onChange={e => {
                setFilterNonMappati(e.target.checked);
                setPage(0);
              }}
            />
            Solo non mappati
                      ...
                      >
                        ...
                        <td style={{ padding: 12, textAlign: 'center' }}>
                          ...
                          {art.mappatura_manuale && (
                            <span title="Mappato manualmente" style={{ marginLeft: 4 }}>
                              ✅
                            </span>
                          )}
                        </td>
