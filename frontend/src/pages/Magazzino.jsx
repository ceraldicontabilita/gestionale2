...
export default function Magazzino() {
  ...
  const stats = useMemo(() => {
    ...
    const lowStock = filteredProducts.filter(
      p => (p.giacenza || 0) <= (p.giacenza_minima || 0) && p.giacenza_minima > 0
    ).length;
    ...
    >
            ...
            >
              ...
              {categories.map(cat => (
                <option key={cat} value={cat}>
                  {cat} ({stats.categorieCounts[cat] || 0})
                </option>
              ))}
            </select>
            <select
      ...
      >
        <div style={{ display: 'flex', borderBottom: '2px solid #e2e8f0' }}>
          ...
          >
            📦 Catalogo Prodotti ({filteredProducts.length}
        ...
        <div style={{ padding: 20 }}>
          {loading ? (
              ...
              <PageEmpty
                ...
                message={
                  catalogProducts.length === 0
                    ? 'I prodotti verranno aggiunti automaticamente dalle fatture XML'
                    ...
                    {filteredProducts.slice(0, 200).map((p, i) => (
                      ...
                      >
                        <td style={{ padding: 12 }}>
                          ...
                          {p.unita_misura && (
                            <span style={{ fontSize: 11, color: '#94a3b8' }}>{p.unita_misura}</span>
                          )}
                        </td>
                        <td style={{ padding: 12 }}>
                          <span
                        ...
                        <td style={{ padding: 12, textAlign: 'right', color: '#16a34a' }}>
                          {formatEuro(p.prezzo_acquisto || p.best_price || 0)}
                        </td>
                        <td style={{ padding: 12, textAlign: 'right', color: '#64748b' }}>
                          {p.prezzo_kg
                            ? `€/kg ${p.prezzo_kg.toFixed(2)}`
                            : formatEuro(p.prezzo_acquisto || 0)}
                        </td>
                        <td style={{ padding: 12, color: '#64748b' }}>
                          {p.ultimo_carico
                            ? formatDateIT(p.ultimo_carico)
                            : p.ultimo_aggiornamento
                              ? formatDateIT(p.ultimo_aggiornamento)
                              : '-'}
                        </td>
                      </tr>
                    ))}
            ...
            <PageEmpty
              icon="📦"
              message="Nessun prodotto nel magazzino manuale. Clicca '+ Nuovo Prodotto' per aggiungerne uno."
            />
          ) : (
            <div style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                  ...
                  {products.map((p, i) => (
                    <tr key={p.id || i} style={{ borderBottom: '1px solid #f1f5f9' }}>
                      <td style={{ padding: 12, fontFamily: 'monospace' }}>
                        {p.code || p.codice_articolo || '-'}
                      </td>
                      <td style={{ padding: 12, fontWeight: 500 }}>{p.name || p.nome || '-'}</td>
                      <td style={{ padding: 12, textAlign: 'right' }}>
                        {p.quantity || p.giacenza || 0} {p.unit || p.unita_misura || ''}
                      </td>
                      <td style={{ padding: 12, textAlign: 'right', color: '#16a34a' }}>
                        {formatEuro(p.unit_price || p.prezzi?.avg || 0)}
                      </td>
