  ...
  const fornitoriFiltered = useMemo(() => {
    ...
    list.sort((a, b) => {
      ...
      if (sortField === 'fornitore') {
        ...
        va = a.saldo;
        vb = b.saldo;
      } else if (sortField === 'dare') {
        va = a.totale_dare;
        vb = b.totale_dare;
      } else if (sortField === 'avere') {
        va = a.totale_avere;
        vb = b.totale_avere;
      } else if (sortField === 'n_doc') {
        va = a.n_documenti;
        vb = b.n_documenti;
      } else {
        va = a.fornitore;
    ...
    for (const f of fornitoriFiltered) {
      rows.push(
        [
          `"${f.fornitore}"`,
          f.partita_iva,
          f.totale_dare.toFixed(2),
          f.totale_avere.toFixed(2),
          f.saldo.toFixed(2),
          f.stato,
          f.n_documenti,
        ].join(';')
    ...
    <>
      ...
      {fornitori?.totali && (
        <PageGrid cols={4} gap={16}>
          <div
            style={{ background: '#f0fdf4', padding: 16, borderRadius: 12, textAlign: 'center' }}
          >
            <div style={{ fontSize: 12, color: '#059669', fontWeight: 600 }}>
              TOTALE DARE (Fatture)
            </div>
            <div style={{ fontSize: 20, fontWeight: 700, color: '#059669', marginTop: 4 }}>
              {formatEuro(fornitori.totali.totale_dare)}
            </div>
          </div>
          <div
            style={{ background: '#fef2f2', padding: 16, borderRadius: 12, textAlign: 'center' }}
          >
            <div style={{ fontSize: 12, color: '#dc2626', fontWeight: 600 }}>
              TOTALE AVERE (Pagamenti)
            </div>
            <div style={{ fontSize: 20, fontWeight: 700, color: '#dc2626', marginTop: 4 }}>
              {formatEuro(fornitori.totali.totale_avere)}
            </div>
          </div>
          <div
            style={{ background: '#fffbeb', padding: 16, borderRadius: 12, textAlign: 'center' }}
          >
            <div style={{ fontSize: 12, color: '#b45309', fontWeight: 600 }}>
              SALDO (Debito residuo)
            </div>
            <div style={{ fontSize: 20, fontWeight: 700, color: '#b45309', marginTop: 4 }}>
              {formatEuro(fornitori.totali.saldo_totale)}
            </div>
          </div>
          <div
            style={{ background: '#f8fafc', padding: 16, borderRadius: 12, textAlign: 'center' }}
          >
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14 }}>
          <thead>
            <tr style={{ background: '#1e293b', color: 'white' }}>
              ...
              <SortHeader field="dare" align="right">
                Dare (Fatture)
              </SortHeader>
              <SortHeader field="avere" align="right">
                Avere (Pagamenti)
              </SortHeader>
              <SortHeader field="saldo" align="right">
                Saldo
              </SortHeader>
              <th style={{ padding: '12px 8px', textAlign: 'center', width: 90 }}>Stato</th>
              <SortHeader field="n_doc" align="center">
                Doc.
              </SortHeader>
            </tr>
          </thead>
            ...
            {fornitoriFiltered.map((f, idx) => {
              ...
              return (
                <React.Fragment key={f.partita_iva}>
                  <tr
                    onClick={() => toggleForn(f.partita_iva)}
                    style={{
                  ...
                  >
                    ...
                    <td
                      style={{
                        ...
                        textAlign: 'right',
                        fontWeight: 700,
                        color: f.saldo > 0 ? '#b45309' : f.saldo < 0 ? '#2563eb' : '#16a34a',
                      }}
                    >
                      {formatEuro(f.saldo)}
                    </td>
                    <td style={{ padding: '10px 8px', textAlign: 'center' }}>
                      {getStatoBadge(f.stato)}
                    </td>
                    <td
                      style={{
                        padding: '10px 8px',
            ...
            <tfoot>
              <tr style={{ background: '#1e293b', color: 'white', fontWeight: 700 }}>
                <td colSpan={3} style={{ padding: '12px 8px' }}>
                  TOTALE ({fornitoriFiltered.length} fornitori)
                </td>
      ...
      <>
        ...
        <PageGrid cols={4} gap={16}>
          ...
          >
            ...
            <div style={{ fontSize: 20, fontWeight: 700, color: '#059669', marginTop: 4 }}>
              {formatEuro(totali.totale_corrispettivi)}
            </div>
          </div>
          <div
            style={{ background: '#eff6ff', padding: 16, borderRadius: 12, textAlign: 'center' }}
          >
            ...
            <div style={{ fontSize: 20, fontWeight: 700, color: '#2563eb', marginTop: 4 }}>
              {formatEuro(totali.totale_fatture_emesse)}
            </div>
          </div>
          <div
            style={{ background: '#fdf4ff', padding: 16, borderRadius: 12, textAlign: 'center' }}
          >
          ...
          >
            ...
            <div style={{ fontSize: 20, fontWeight: 700, color: '#b45309', marginTop: 4 }}>
              {formatEuro(totali.media_giornaliera)}
            </div>
          </div>
        </PageGrid>
        ...
        {fatture_emesse?.length > 0 && (
          <PageSection
            title={`Fatture Emesse a Clienti (${fatture_emesse.length})`}
        ...
        >
          <p style={{ margin: 0, fontSize: 12, color: '#1e40af' }}>
            <strong>Nota HORECA:</strong> Per le attività al dettaglio (ristoranti, bar) i clienti
            sono prevalentemente anonimi. I corrispettivi rappresentano vendite con incasso
            contestuale, quindi Dare = Avere (saldo zero). Le eventuali fatture emesse a clienti
            sono già incluse nei corrispettivi e servono solo per la detrazione IVA del cliente.
          </p>
        </div>
      </>
    );
