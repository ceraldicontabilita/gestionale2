  ...
  const loadDettaglio = async employeeId => {
    ...
    try {
      const res = await api.get(
        `/api/giustificativi/riepilogo-progressivo/${employeeId}?anno=${selectedYear}`
      );
        ...
        <>
                  ...
                  <tbody>
                    {dipendentiConSaldi.map((dip, idx) => (
                      ...
                      >
                        ...
                        <td style={{ padding: '12px 16px', textAlign: 'center' }}>
                          {dip.periodo ? (
                            <span style={{ fontSize: 13, color: '#64748b' }}>{dip.periodo}</span>
                          ) : (
                        ...
                        <td style={{ padding: '12px 16px', textAlign: 'center' }}>
                          ...
                          >
                            ...
                            <Button
                              ...
                              onClick={() => {
                                const nuovoPeriodo = prompt(
                                  'Modifica periodo (es. 2026-02):',
                                  dip.periodo || ''
                                );
                                if (nuovoPeriodo && nuovoPeriodo !== dip.periodo) {
                                  api
                                    .put(`/api/giustificativi/saldi-finali/${dip.id}/periodo`, {
                                      anno: selectedYear,
                            ...
                            <Button
                              ...
                              onClick={() => {
                                if (
                                  window.confirm(`Eliminare i saldi ferie/permessi di ${dip.nome}?`)
                                ) {
                                  api
                                    .delete(
                                      `/api/giustificativi/saldi-finali/${dip.id}?anno=${selectedYear}`
                                    )
                                    .then(() => {
                                      loadSaldi();
                                    })
          ...
          {dettaglio && (
            <Card style={{ marginTop: 20 }}>
              <CardHeader>
                <CardTitle>Dettaglio: {dettaglio.employee_nome}</CardTitle>
                <CardDescription>
            ...
            >
              ...
              >
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  <div>
                    <label
                      style={{ display: 'block', fontSize: 13, color: '#64748b', marginBottom: 4 }}
                    >
                      Ferie (ore)
                    </label>
                    <input
                ...
                <div style={{ display: 'flex', gap: 12, marginTop: 24 }}>
                  ...
                  <Button onClick={saveSaldi} disabled={saving} style={{ flex: 1 }}>
