...
export default function Riconciliazione() {
  ...
  useEffect(() => {
    if (activeTab === 'manuale') {
      ...
      Promise.all([loadMovimentiNonRiconciliati(), loadFattureNonPagate()]).catch(e =>
        console.error('Errore caricamento dati manuali:', e)
      );
    }
  }, [activeTab, anno]);
  ...
  const loadStats = async () => {
    ...
    setLoadError('');
    try {
      const res = await api.get(`/api/riconciliazione-auto/stats-riconciliazione?anno=${anno}`, {
        timeout: 30000,
      });
      setStats(res.data);
    } catch (e) {
  ...
  const loadMovimentiNonRiconciliati = async () => {
    try {
      const res = await api.get(
        `/api/estratto-conto-movimenti/movimenti?riconciliato=false&limit=200&anno=${anno}`
      );
  ...
  const handleSelectMovimento = mov => {
    ...
    const matching = fattureNonPagate
      .filter(f => {
  ...
  return (
    <PageLayout
      ...
      subtitle="Associa i movimenti dell'estratto conto con fatture, POS, versamenti e F24"
    >
      <div style={{ ...pageStyle.container, padding: 0 }} data-testid="riconciliazione-page">
        ...
        <div style={pageStyle.header}>
          ...
          <div style={{ display: 'flex', gap: '10px' }}>
            <button style={pageStyle.button('outline')} onClick={loadStats} disabled={loading}>
              🔄 Aggiorna
            </button>
          </div>
        </div>
        ...
        {stats && (
          <div style={pageStyle.statsGrid}>
            ...
            <div style={pageStyle.statCard('#10b981')}>
              <p style={pageStyle.statValue('#10b981')}>
                {stats.estratto_conto?.riconciliati || 0}
              </p>
                        ...
                        {matchingFatture.map(f => (
                          <tr key={f._id || f.id}>
                            ...
                            <td style={pageStyle.td}>
                              {f.invoice_number || f.numero_fattura || '-'}
                            </td>
